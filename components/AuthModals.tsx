...
import { legalDE } from '../i18n/legal/legal.de';
...
const TroubleshootingBox: React.FC<{ code: string }> = ({ code }) => {
    ...
    return (
        <div className="mt-4 p-6 bg-black/40 border-2 border-hey-church-orange-500/50 rounded-[2rem] space-y-5 animate-in fade-in slide-in-from-top-2">
            <div className="flex items-center text-hey-church-orange-400 font-black text-xs uppercase tracking-widest">
            ...
            {code === 'auth/unauthorized-domain' ? (
                <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="bg-gray-900 border border-white/10 p-1.5 rounded-xl flex items-center justify-between group">
                            <code className="px-3 text-xs text-hey-church-blue font-mono truncate">{host}</code>
                            <button
                                onClick={handleCopyDomain}
                                className={`px-4 py-2 rounded-lg transition-all font-black text-[10px] uppercase tracking-widest flex items-center ${copied ? 'bg-green-500 text-white' : 'bg-hey-church-blue text-white hover:bg-blue-600'}`}
                            >
                    ...
                    <div className="space-y-2">
                        ...
                        <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="w-full py-4 bg-hey-church-orange-500 text-white rounded-2xl flex items-center justify-center font-black text-xs uppercase tracking-widest shadow-lg hover:brightness-110 transition-all border-b-4 border-hey-church-orange-700 active:border-b-0 active:translate-y-1"
                        >
                ...
                <div className="space-y-3">
                    ...
                    <a href="https://console.firebase.google.com/" target="_blank" rel="noreferrer" className="block text-center text-[10px] text-hey-church-blue font-bold uppercase underline">Open Firebase Console</a>
                </div>
            )}
        </div>
    );
...
export const SignUpModal: React.FC<SignUpModalProps> = ({ onClose, onSwitchToSignIn }) => {
    ...
    const privacyText = legalDE.privacy;
    const termsText = legalDE.terms;
...
React.useEffect(() => {
  if (acceptedPrivacy && acceptedTerms) {
    ...
    return (
        <div className="fixed inset-0 bg-hey-church-bg/95 backdrop-blur-md flex items-center justify-center p-4 z-50 overflow-y-auto">
            <div className="bg-gray-800 rounded-[2.5rem] shadow-2xl max-w-md w-full relative border border-white/5 overflow-hidden my-8">
                ...
                <div className="p-10 space-y-8">
                    ...
                    <form className="space-y-4" onSubmit={handleSubmit}>
                        <input type="text" placeholder="Your Name" value={firstName} onChange={e => setFirstName(e.target.value)} className="w-full p-4 bg-gray-900 border border-gray-700 rounded-2xl text-white outline-none focus:border-hey-church-red transition-colors" required />
                        <input type="email" placeholder="Email" value={email} onChange={e => setEmail(e.target.value)} className="w-full p-4 bg-gray-900 border border-gray-700 rounded-2xl text-white outline-none focus:border-hey-church-red transition-colors" required />
                        <input type="password" placeholder="Password" value={password} onChange={e => setPassword(e.target.value)} className="w-full p-4 bg-gray-900 border border-gray-700 rounded-2xl text-white outline-none focus:border-hey-church-red transition-colors" required />
                        <label className="flex items-start space-x-3 text-xs text-gray-300">
                            <input
                                ...
                                onChange={(e) => {
                                    ...
                                    // If user has NOT accepted both, force modal
                                    if (checked && (!acceptedPrivacy || !acceptedTerms)) {
                                        if (!acceptedPrivacy) {
                                            setOpenPrivacyModal(true);
                                        } else if (!acceptedTerms) {
                                            setOpenTermsModal(true);
                                        }
                                        return; // ❗ do NOT check the box yet
                                    }

                                    // Otherwise allow normal toggle
                                    setAgreedMain(checked);
                                }}
                                className="mt-1"
                            />
                        ...
                        <button type="submit"
                            disabled={
                                ...
                                !acceptedPrivacy ||
                                !acceptedTerms
                            } className="w-full bg-hey-church-red py-5 rounded-2xl font-black text-white shadow-xl hover:brightness-110 active:scale-95 transition-all uppercase tracking-widest">
                            {loading ? <Loader2 className="animate-spin mx-auto" /> : 'Create Account'}
                        </button>
                    </form>

                    <button onClick={onSwitchToSignIn} className="w-full text-center text-xs font-bold text-hey-church-orange-400 hover:text-white transition-colors">
            ...
            {openPrivacyModal && (
                <TermsModal
                    title="Datenschutzerklärung – Hey Life"
                    content={privacyText}
            ...
            {openTermsModal && (
                <TermsModal
                    title="Nutzungsbedingungen – Hey Life"
                    content={termsText}
            ...
            <div className="bg-gray-800 rounded-[2.5rem] shadow-2xl max-w-md w-full relative border border-white/5 overflow-hidden my-8">
                ...
                <div className="p-10 space-y-8">
                    ...
                    <form className="space-y-4" onSubmit={handleSubmit}>
                        <input type="email" placeholder="Email" value={email} onChange={e => setEmail(e.target.value)} className="w-full p-4 bg-gray-900 border border-gray-700 rounded-2xl text-white outline-none focus:border-hey-church-red transition-colors" required />
                        <input type="password" placeholder="Password" value={password} onChange={e => setPassword(e.target.value)} className="w-full p-4 bg-gray-900 border border-gray-700 rounded-2xl text-white outline-none focus:border-hey-church-red transition-colors" required />
                        <button type="submit" disabled={loading} className="w-full bg-hey-church-red py-5 rounded-2xl font-black text-white shadow-xl hover:brightness-110 active:scale-95 transition-all uppercase tracking-widest">
                            {loading ? <Loader2 className="animate-spin mx-auto" /> : 'Sign In'}
                        </button>
                    </form>

                    <button onClick={onSwitchToSignUp} className="w-full text-center text-xs font-bold text-hey-church-orange-400 hover:text-white transition-colors">
