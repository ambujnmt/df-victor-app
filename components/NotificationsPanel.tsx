...
const NotificationCard: React.FC<{ notification: AppNotification; onClosePanel: () => void }> = ({ notification, onClosePanel }) => {
    ...
    return (
        <div className={`p-4 rounded-xl border ${notification.isRead ? 'bg-gray-800 border-gray-700/50' : 'bg-hey-church-blue/10 border-hey-church-blue/30 shadow-lg'}`}>
            ...
            {(notification.type === 'challenge_invitation' || notification.type === 'group_invitation') && !notification.isRead && (
                <div className="flex space-x-2 mt-4">
                    <button onClick={() => handleResponse(true)} className="flex-1 bg-hey-church-green hover:brightness-110 text-white font-black py-2.5 px-3 text-[10px] uppercase tracking-widest rounded-lg flex items-center justify-center shadow-lg transition-all active:scale-95">
            ...
            {notification.type === 'group_challenge_suggestion' && !notification.isRead && (
                 <button onClick={handleViewChallenge} className="w-full mt-4 bg-hey-church-orange-500 text-white font-black py-2.5 px-3 text-[10px] uppercase tracking-widest rounded-lg flex items-center justify-center shadow-lg">
                    View Challenge <ArrowRight size={14} className="ml-1.5"/>
                </button>
            )}
            {notification.type === 'lesson_invitation' && (
                 <button onClick={handleViewLesson} className="w-full mt-4 bg-hey-church-red text-white font-black py-2.5 px-3 text-[10px] uppercase tracking-widest rounded-lg flex items-center justify-center shadow-lg">
                    Watch Lesson <PlayCircle size={14} className="ml-1.5"/>
                </button>
            )}
            {notification.type === 'course_invitation' && (
                 <button onClick={handleViewCourse} className="w-full mt-4 bg-hey-church-purple text-white font-black py-2.5 px-3 text-[10px] uppercase tracking-widest rounded-lg flex items-center justify-center shadow-lg">
        ...
        <div className={`fixed top-0 right-0 h-full w-full max-w-md bg-gray-900 shadow-2xl z-50 transform transition-transform duration-500 ease-in-out border-l border-white/5 ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
            <div className="flex flex-col h-full">
                <div className="p-6 border-b border-gray-800 flex justify-between items-center flex-shrink-0 bg-gray-900/50 backdrop-blur-md">
                    <h3 className="text-xl font-black uppercase italic flex items-center"><Bell className="mr-3 text-hey-church-yellow" size={24}/> Notifications</h3>
                ...
                <div className="p-6 space-y-4 overflow-y-auto flex-grow bg-hey-church-bg/30">
