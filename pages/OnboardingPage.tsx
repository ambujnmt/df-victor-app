...
const OnboardingPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-hey-church-bg text-white flex flex-col p-6 relative overflow-hidden">
        {/* Progress Bar */}
        <div className="w-full bg-gray-800 h-2 rounded-full mb-8 z-10">
            <div 
                className="bg-hey-church-red h-2 rounded-full transition-all duration-500 ease-out" 
            ...
            {step < totalQuestionSteps ? (
                            ...
                            return (
                                <button
                                    ...
                                    className={`w-full text-left p-4 rounded-xl border-2 transition-all duration-200 flex justify-between items-center group ${
                                        isSelected 
                                        ? 'bg-hey-church-red/20 border-hey-church-red' 
                                        : 'bg-gray-800 border-gray-700 hover:border-hey-church-orange-700'
                                    }`}
                                >
                                    <span className="font-bold text-lg">{label}</span>
                                    {isSelected && <div className="bg-hey-church-red rounded-full p-1"><ArrowRight size={14} /></div>}
                                </button>
                            );
                        })}
                    </div>
                    {currentQuestion.multiSelect && (
                        <button onClick={handleNext} className="mt-6 w-full bg-white text-hey-church-bg font-bold py-3 rounded-lg">
                            Next
                        </button>
                    )}
                </div>
                ...
                <div className="text-center animate-in fade-in zoom-in duration-500">
                    <div className="bg-gray-800 inline-block p-6 rounded-full mb-6 shadow-xl">
                        <Bell size={48} className="text-hey-church-yellow animate-bounce-slow" />
                    </div>
                    ...
                    <button onClick={handleNext} className="w-full bg-hey-church-blue text-white font-bold py-4 rounded-xl shadow-lg mb-3 hover:scale-105 transition-transform">
