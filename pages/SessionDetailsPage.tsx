        ...
        <div className="bg-gray-800/50 border border-white/10 rounded-2xl p-5 mb-6 animate-in slide-in-from-right-4">
            <div className="flex justify-between items-start mb-4">
                <h4 className="font-bold text-white flex items-start">
                    <HelpCircle size={18} className="mr-2 mt-1 text-hey-church-blue flex-shrink-0" />
                    {question.question}
                </h4>
            </div>
            ...
            <div className="space-y-3">
                {question.options.map((opt, i) => {
                    ...
                    else if (isSelected && !isRight) style = "bg-hey-church-blue/20 border-hey-church-blue text-white";
            ...
            {isRight && question.explanation && (
                <div className="mt-5 bg-gradient-to-r from-hey-church-orange-900/30 to-hey-church-yellow/10 border border-hey-church-yellow/20 rounded-xl p-4 animate-in fade-in zoom-in duration-500">
                    <p className="text-[10px] font-black text-hey-church-yellow uppercase tracking-widest flex items-center mb-2">
...
const SessionDetailsPage: React.FC = () => {
  ...
  return (
    <>
    <div className="p-4 bg-hey-church-bg min-h-screen text-white flex flex-col pb-20">
      <div className="flex justify-between items-start mb-4">
          <button onClick={() => navigate('/academy')} className="text-hey-church-orange-400 font-bold flex items-center">
              &larr; Back
          </button>
          <button 
            onClick={() => setShowInviteModal(true)}
            className="flex items-center bg-hey-church-blue/20 text-hey-church-blue px-3 py-1.5 rounded-full text-xs font-bold hover:bg-hey-church-blue/40 transition-colors"
          >
      ...
      <div className="flex bg-gray-800 p-1 rounded-lg mb-4">
          <button onClick={() => setActiveTab('study')} className={`flex-1 py-2 rounded font-bold text-sm transition-colors ${activeTab === 'study' ? 'bg-hey-church-blue text-white' : 'text-gray-400'}`}>Study</button>
          <button onClick={() => setActiveTab('reflect')} className={`flex-1 py-2 rounded font-bold text-sm transition-colors ${activeTab === 'reflect' ? 'bg-hey-church-blue text-white' : 'text-gray-400'}`}>Reflect</button>
          {session.downloadUrl && <button onClick={() => setActiveTab('resources')} className={`flex-1 py-2 rounded font-bold text-sm transition-colors ${activeTab === 'resources' ? 'bg-hey-church-blue text-white' : 'text-gray-400'}`}>Resources</button>}
      </div>
          ...
          {activeTab === 'study' && (
              <div className="space-y-4 animate-in fade-in">
                  {session.keyTakeaways && session.keyTakeaways.length > 0 && (
                      <div className="bg-gray-800 p-5 rounded-2xl border-l-4 border-hey-church-yellow shadow-lg">
                          <h3 className="font-black text-xs uppercase tracking-widest mb-3 text-hey-church-yellow">Key Takeaways</h3>
                          <ul className="space-y-3">
                              {session.keyTakeaways.map((k, i) => (
                                  <li key={i} className="flex items-start text-gray-200 text-sm">
                                      <CheckCircle size={14} className="mr-2 mt-1 text-hey-church-green flex-shrink-0" />
                                      {k}
                                  </li>
                              ))}
                          </ul>
                  ...
                  {session.caseStudy && (
                      <div className="bg-gradient-to-br from-gray-800 to-gray-900 p-5 rounded-2xl border border-white/5 shadow-xl">
                          <h3 className="font-bold mb-3 flex items-center text-hey-church-blue uppercase text-xs tracking-widest"><BrainCircuit size={16} className="mr-2"/> Case Study</h3>
                          ...
                          <textarea 
                              className="w-full bg-gray-900 border border-gray-700 rounded-xl p-4 text-white h-28 focus:ring-2 focus:ring-hey-church-blue focus:outline-none placeholder-gray-600 text-sm"
              ...
              <div className="space-y-6 animate-in fade-in">
                  ...
                  {session.bibleVerses && session.bibleVerses.length > 0 && (
                      <div className="bg-gray-800/40 rounded-2xl p-5 border border-white/5 shadow-md">
                          <h3 className="font-black text-xs uppercase tracking-widest mb-4 flex items-center text-hey-church-yellow">
                  ...
                  {session.quizQuestions && session.quizQuestions.length > 0 && (
                      <div className="space-y-2">
                          <h3 className="font-black text-xs uppercase tracking-widest mb-4 text-hey-church-blue px-2">Knowledge Check</h3>
                  ...
                  {session.reflectionQuestions.length > 0 && (
                    <div className="space-y-4">
                        <h3 className="font-black text-xs uppercase tracking-widest mb-4 text-hey-church-red px-2">Heart Reflection</h3>
                        {session.reflectionQuestions.map((q, i) => (
                            <div key={i} className="bg-gray-800/30 p-5 rounded-2xl border border-white/5">
                                ...
                                <textarea 
                                    className="w-full bg-gray-900 border border-gray-700 rounded-xl p-4 text-white h-24 focus:ring-2 focus:ring-hey-church-red focus:outline-none placeholder-gray-600 text-sm"
                  ...
                  <div className="pt-4">
                      {isCompleted ? (
                          <div className="bg-green-500/20 border border-green-500 rounded-2xl p-6 text-center animate-in zoom-in">
                              <Trophy size={48} className="mx-auto text-hey-church-yellow mb-3" />
                          ...
                          <button 
                              ...
                              className="w-full bg-hey-church-red py-5 rounded-2xl font-black text-lg shadow-[0_10px_30px_rgba(239,109,77,0.4)] hover:scale-[1.02] active:scale-95 transition-all disabled:opacity-50 disabled:grayscale"
                          >
