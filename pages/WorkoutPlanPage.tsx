                ...
                <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-3 gap-3">
                            {['Too Easy', 'Just Right', 'Too Hard'].map(level => (
                                <button 
                                    ...
                                    className={`p-4 rounded-xl text-sm font-bold border-2 transition-all ${difficulty === level ? 'bg-hey-church-red text-white border-hey-church-red transform scale-105 shadow-lg' : 'border-gray-600 text-gray-400 hover:border-gray-500'}`}
                                >
                                    {level}
                                </button>
                            ))}
                    ...
                    <div>
                        ...
                        <textarea 
                            ...
                            className="w-full p-3 h-24 bg-gray-900 border border-gray-700 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-hey-church-blue"
                        />
                    </div>
                    <button onClick={handleSubmit} className="w-full bg-hey-church-green text-white font-bold py-4 rounded-xl text-lg hover:bg-opacity-90 transition shadow-lg flex items-center justify-center">
    ...
    return (
        <div className="bg-gradient-to-br from-gray-800 to-gray-900 rounded-2xl shadow-xl overflow-hidden border border-gray-700">
            <div className="bg-[#133666] p-4 flex justify-between items-center">
                ...
                <button onClick={refreshInsight} disabled={isGenerating} className="text-hey-church-orange-400 hover:text-white transition-colors disabled:opacity-50">
            ...
            <div className="p-6">
                {isGenerating ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-4 text-center">
                        <AlertCircle className="text-hey-church-red mb-2" size={24} />
                        <p className="text-xs text-gray-400">{error}</p>
                    </div>
                ) : insight ? (
                    <div>
                        <div className="flex items-start mb-4">
                            <MessageSquareQuote className="text-hey-church-red mr-3 flex-shrink-0 mt-1" size={24} />
...
const WorkoutPlanPage: React.FC<WorkoutPlanPageProps> = () => {
  ...
  if (!plan) {
    return (
      <div className="min-h-screen bg-hey-church-bg flex flex-col items-center justify-center p-6 text-white text-center">
        ...
        <button onClick={() => navigate('/my-plan')} className="mt-4 bg-hey-church-red font-bold py-3 px-6 rounded-full">Go to Profile</button>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 pb-24 space-y-6 bg-hey-church-bg min-h-screen text-white">
      {/* Header */}
      <div className="flex items-center justify-between">
          <button onClick={() => navigate('/my-plan')} className="flex items-center text-hey-church-orange-400 font-bold">
      ...
      {activeTab === 'coach' && (
          <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                  <div className="bg-gray-800 p-5 rounded-2xl shadow-lg border-l-4 border-hey-church-green">
                   ...
                   <div className="bg-gray-800 p-5 rounded-2xl shadow-lg border-l-4 border-hey-church-yellow">
              ...
              <div className="bg-gray-800 p-6 rounded-2xl shadow-lg">
                  <h3 className="font-bold text-lg mb-4 flex items-center"><Info size={20} className="mr-2 text-hey-church-blue"/> Program Stats</h3>
                  <div className="space-y-4">
                      <div>
                          ...
                          <div className="w-full bg-gray-700 rounded-full h-2">
                              <div className="bg-hey-church-red h-2 rounded-full" style={{ width: `${(activeWeek / plan.duration) * 100}%` }}></div>
                          </div>
                      </div>
                  </div>
              </div>
                    ...
                    {plan.weeks.map(week => (
                        <button
                            ...
                            className={`flex-shrink-0 w-16 h-16 flex flex-col items-center justify-center rounded-2xl transition-all border-2 ${
                                ...
                                ? 'bg-hey-church-red border-hey-church-red text-white shadow-lg scale-105' 
                    ...
                    <div key={day.day} className="bg-gray-800 rounded-2xl shadow-lg overflow-hidden border border-gray-700/50">
                        <div className="p-5 flex justify-between items-start">
                            <div>
                                ...
                                <p className="text-hey-church-orange-400 font-bold">{day.title}</p>
                            </div>
                            ...
                            <>
                                        ...
                                        <div key={idx} className="bg-gray-900/50 p-3 rounded-xl flex justify-between items-center">
                                            <div>
                                                ...
                                                <p className="text-xs text-gray-400 mt-1">
                                                    {ex.sets} sets × {ex.reps} • <span className="text-hey-church-yellow">{ex.weight}</span>
                                                </p>
                                            </div>
                                ...
                                <div className="bg-gray-900/30 p-4 border-t border-gray-700">
                                    <button
                                        ...
                                        className="w-full bg-hey-church-green text-white font-bold py-3 px-4 rounded-xl text-sm hover:bg-opacity-90 transition-transform active:scale-95 flex items-center justify-center shadow-md"
                                    >
