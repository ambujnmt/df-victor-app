...
const ChallengeCard: React.FC<{
  ...
  return (
    ...
    >
        ...
        <div className="absolute top-4 right-4 bg-hey-church-yellow text-gray-900 text-xs font-bold px-2.5 py-1 rounded-full">
            +{challenge.points} Points
        </div>

        <div className="flex-grow pr-20"> {/* Add padding to the right to avoid overlap with points */}
            <h3 className="font-extrabold text-xl text-white">{challenge.title}</h3>
            <p className="text-xs text-hey-church-orange-400 font-semibold uppercase tracking-wider mt-1">{challenge.category}</p>
        ...
        <div className="flex items-center justify-between pt-4 mt-3 border-t border-gray-700/50">
            ...
            <div className="flex items-center space-x-2">
              <button 
                ...
                className="font-semibold py-2 px-4 rounded-lg flex items-center justify-center text-sm bg-hey-church-blue text-white hover:bg-opacity-80 transition-transform hover:scale-105"
              >
              ...
              {isActive ? (
                ...
                <button 
                  ...
                  disabled={isStarting}
                  className="font-bold py-2 px-5 rounded-lg flex items-center justify-center text-sm bg-hey-church-red text-white hover:bg-opacity-90 transition-transform hover:scale-105 disabled:bg-gray-600 disabled:scale-100"
                >
  ...
  return (
    <>
      <div className="p-4 space-y-6 bg-hey-church-bg min-h-screen text-white">
        <h1 className="text-3xl font-extrabold uppercase">{t('challenges.title')}</h1>
        <p className="text-gray-300">{t('challenges.subtitle')}</p>
        ...
        <div className="bg-gray-800 p-1 rounded-xl shadow-md flex justify-between space-x-1 overflow-x-auto">
          {durations.map(day => (
            <button
              ...
              className={`flex-1 min-w-[60px] py-2 text-sm font-bold rounded-lg transition-colors ${durationFilter === day ? 'bg-hey-church-red text-white' : 'text-gray-300 hover:bg-gray-700'}`}
            >
