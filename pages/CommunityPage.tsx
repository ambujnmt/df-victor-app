    ...
    return (
        <div className="bg-gray-800 rounded-xl shadow-lg p-5 space-y-3">
            <div className="flex items-center mb-3">
                <img src={displayAvatar} alt={displayName} className="w-12 h-12 rounded-full mr-4 border-2 border-hey-church-purple object-cover" />
                <div>
            ...
            <div className="flex items-center justify-between pt-3 border-t border-gray-700">
                <div className="flex items-center space-x-2 text-hey-church-orange-400">
                    <Heart size={18} />
                ...
                <button
                    ...
                    disabled={hasPrayed}
                    className={`font-bold py-2 px-4 rounded-lg flex items-center justify-center transition-all text-sm ${
                        hasPrayed
                            ? 'bg-gray-600 text-gray-400 cursor-not-allowed'
                            : 'bg-hey-church-red text-white hover:bg-opacity-90 hover:scale-105'
                    }`}
                >
    ...
    <div className="bg-gray-800 rounded-xl shadow-lg overflow-hidden border border-gray-700">
      <div className="p-4">
        <div className="flex items-center mb-4">
          <img src={displayAvatar} alt={post.user.name} className="w-12 h-12 rounded-full mr-4 border-2 border-hey-church-orange-500 object-cover" />
          <div>
              ...
              {post.comments.map((comment, idx) => (
                  <div key={idx} className="flex items-start space-x-2 text-xs">
                      ...
                      <div className="flex-grow">
                          <span className="font-bold text-hey-church-orange-300 mr-1.5">{comment.user.name}:</span>
...
const HeyFamCard: React.FC<{ fam: HeyFam }> = ({ fam }) => (
    <Link to={`/community/fam/${fam.id}`} className="block bg-gray-800 p-4 rounded-lg shadow-md hover:bg-gray-700 transition-colors">
        <div className="flex items-center space-x-4">
            <img src={fam.avatar} alt={fam.name} className="w-16 h-16 rounded-full border-2 border-hey-church-blue object-cover"/>
            <div>
    ...
    return (
        <div className="space-y-4">
            <div className="flex bg-gray-900 rounded-lg p-1">
                <button onClick={() => setFilter('all')} className={`flex-1 font-bold py-2 text-sm rounded-md ${filter === 'all' ? 'bg-hey-church-red' : ''}`}>{t('community.leaderboard.all_time')}</button>
                <button onClick={() => setFilter('my_fams')} className={`flex-1 font-bold py-2 text-sm rounded-md ${filter === 'my_fams' ? 'bg-hey-church-red' : ''}`}>{t('community.leaderboard.my_fams')}</button>
            </div>
            {leaderboardData.length > 0 ? (
                <div className="space-y-2">
                    {leaderboardData.map(user => {
                        ...
                        return (
                            <div key={user.id} className={`flex items-center p-3 rounded-lg ${user.id === currentUser.id ? 'bg-hey-church-orange-900/50 border-2 border-hey-church-orange-400' : 'bg-gray-900'}`}>
                                ...
                                <p className="font-bold text-hey-church-yellow">{user.points} pts</p>
                            </div>
                        );
                    })}
                </div>
...
const TabButton: React.FC<{ icon: React.ElementType; label: string; active: boolean; onClick: () => void; }> = ({ icon: Icon, label, active, onClick }) => (
  <button
    ...
    className={`flex flex-col items-center justify-center p-3 rounded-xl transition-all duration-200
      ${active ? 'bg-hey-church-red text-white scale-105 shadow-lg' : 'bg-gray-800 text-gray-400 hover:bg-gray-700 hover:text-white'}`}
  >
    <Icon size={24} />
...
const handleAddComment = (postId: string, newComment: any) => {
  ...
  return (
    <>
    <div className="p-4 space-y-6 bg-hey-church-bg min-h-screen text-white">
      ...
      {activeTab === 'feed' && (
        <div className="space-y-4 pb-20">
            <div className="bg-gray-800 p-4 rounded-xl shadow-lg border border-gray-700">
                <textarea
                className="w-full p-3 bg-gray-700 border border-gray-600 rounded-md text-white placeholder-gray-400 focus:ring-2 focus:ring-hey-church-red focus:border-hey-church-red"
                ...
                className="mt-3 w-full bg-hey-church-red text-white font-bold py-3 px-4 rounded-lg hover:bg-opacity-90 flex items-center justify-center transition"
                >
      ...
      {activeTab === 'prayer' && (
        <div className="space-y-6 pb-20">
            <button 
                ...
                className="w-full bg-gradient-to-r from-blue-900 to-hey-church-blue rounded-xl p-6 shadow-lg flex items-center justify-between group hover:scale-[1.01] transition-transform"
            >
            ...
            <div className="bg-gray-800 p-4 rounded-xl shadow-lg border border-gray-700">
                <h2 className="font-bold text-lg mb-2 flex items-center"><HandHelping className="mr-2 text-hey-church-orange-400"/> {t('community.prayer.title')}</h2>
                <textarea
                    className="w-full h-24 p-3 bg-gray-700 border border-gray-600 rounded-md text-white placeholder-gray-400 focus:ring-2 focus:ring-hey-church-red focus:border-hey-church-red"
                ...
                <div className="flex justify-between items-center mt-3">
                    <label className="flex items-center text-sm text-gray-400 select-none">
                        <input type="checkbox" checked={isAnonymous} onChange={(e) => setIsAnonymous(e.target.checked)} className="mr-2 h-4 w-4 rounded bg-gray-700 border-gray-600 text-hey-church-red focus:ring-hey-church-red" />
                        {t('community.prayer.anonymous')}
                    </label>
                    <button onClick={handleSubmitRequest} disabled={!newRequestText.trim()} className="bg-hey-church-red text-white font-bold py-2 px-4 rounded-lg flex items-center justify-center transition disabled:bg-gray-600">
