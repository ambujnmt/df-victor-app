...
import { Award, Bot, Settings, Dumbbell, Trophy, Users, GraduationCap, Shield, Footprints, Sparkles, Camera, Loader2, BookOpen, ChevronRight, HeartHandshake, QrCode, Lock, X, Gift, FileText, CheckCircle2, Activity } from 'lucide-react';
...
const RecommendationCard: React.FC<{ recommendation: Recommendation }> = ({ recommendation }) => {
    const { t } = useLanguage();
    return (
        <div className="bg-hey-church-purple text-white rounded-xl shadow-lg p-6">
            <h3 className="text-xl font-bold mb-2 flex items-center"><Sparkles className="mr-3 text-hey-church-yellow"/> {t('recommendation.title')}</h3>
            ...
            <Link to={recommendation.link} className="w-full mt-4 inline-flex items-center justify-center bg-white text-hey-church-purple font-bold py-3 px-4 rounded-full transition-transform hover:scale-105">
...
const BadgeDisplay: React.FC<{ badge: Badge & {isEarned: boolean}, t: (key: string, options?: any) => string }> = ({ badge, t }) => {
    ...
    return (
        <div className={`flex flex-col items-center justify-center p-3 rounded-2xl w-24 h-28 flex-shrink-0 transition-all ${badge.isEarned ? 'bg-gradient-to-br from-[#1e4b8a] to-[#133666] border border-hey-church-yellow/30 shadow-lg' : 'bg-gray-800/50 opacity-60 grayscale'}`}>
            <Icon size={28} className={`mb-2 ${badge.isEarned ? 'text-hey-church-yellow' : 'text-gray-400'}`} />
    ...
    <div className="p-4 pb-24 space-y-6 bg-gray-900 min-h-screen text-white font-sans">
      ...
      <div className="relative pt-4 pb-6 flex flex-col items-center">
        ...
        <div className="relative group">
            <div className="w-28 h-28 rounded-full p-1 bg-gradient-to-tr from-hey-church-red to-hey-church-orange-400">
                <img src={user.avatar} alt={user.name} className="w-full h-full rounded-full border-4 border-gray-900 object-cover" />
            </div>
            <button onClick={handleAvatarClick} className="absolute bottom-1 right-1 bg-[#133666] p-2 rounded-full text-white hover:bg-hey-church-red transition-colors border-2 border-gray-900 shadow-sm" disabled={isUploading}>
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <ActionCard title={t('my_plan.journal.title')} icon={BookOpen} actionLabel={t('my_plan.journal.open')} onAction={() => navigate('/journal')} className="bg-gradient-to-br from-[#133666] to-[#0f2a50]">
                ...
                <div className="flex items-center text-xs text-hey-church-orange-400 font-bold"><span className="bg-white/10 px-2 py-1 rounded-md">🔥 {user.journalStreak || 0} Day Streak</span></div>
            </ActionCard>

            <ActionCard title={t('my_plan.workout.title')} icon={Dumbbell} actionLabel={user.workoutPlan ? t('my_plan.workout.view_plan') : t('my_plan.workout.create_plan')} onAction={() => navigate(user.workoutPlan ? '/workout-plan' : '/workout-onboarding')} className="bg-hey-church-blue">
                {user.workoutPlan ? (
                    <div>
        ...
        <div className="bg-[#133666] rounded-2xl p-5 shadow-lg flex flex-col">
            <div className="flex justify-between items-center mb-4">
                <h3 className="font-bold text-lg text-white uppercase flex items-center">
                    <Trophy className="mr-2 text-hey-church-yellow" size={20}/> My Rewards & Activity
                </h3>
            </div>
            <div className="grid grid-cols-2 gap-4 mb-6">
                <div className="bg-gray-900/40 p-4 rounded-xl text-center border border-white/5">
                    ...
                    <p className="text-2xl font-extrabold text-hey-church-yellow">{user.points.toLocaleString()}</p>
                </div>
                <div className="bg-gray-900/40 p-4 rounded-xl text-center border border-white/5">
                    <p className="text-[10px] text-gray-400 uppercase font-black tracking-widest mb-1">Badges Earned</p>
                    <p className="text-2xl font-extrabold text-hey-church-blue">{earnedBadgesCount}</p>
                </div>
            </div>
            ...
            <div className="bg-gray-900/30 p-4 rounded-xl mb-4">
                <div className="flex items-center justify-between mb-3">
                    ...
                    <Activity size={14} className="text-hey-church-orange-400" />
                </div>
                <div className="flex overflow-x-auto space-x-3 pb-2 scrollbar-hide">
                    {earnedBadges.filter(b => b.isEarned).slice(0, 5).map(badge => (
                        <div key={badge.id} className="flex-shrink-0 w-12 h-12 bg-white/5 rounded-full flex items-center justify-center border border-white/10">
                            {React.createElement(badgeIcons[badge.icon] || Award, { size: 20, className: "text-hey-church-yellow" })}
                        </div>
                    ))}
            ...
            {user.certificates && user.certificates.length > 0 ? (
                <div className="grid grid-cols-2 gap-3">
                    {user.certificates.map(cert => (
                        ...
                        >
                            <div className="w-12 h-12 bg-hey-church-yellow/20 rounded-full flex items-center justify-center mb-3 text-hey-church-yellow group-hover:scale-110 transition-transform">
                                <Award size={24} />
                            </div>
                            <p className="text-[10px] font-black uppercase text-hey-church-orange-400 mb-1 leading-tight">{cert.courseTitle}</p>
        ...
        <ActionCard title={t('my_plan.ai.title')} icon={Bot} className="bg-[#133666]">
            <p className="text-sm text-gray-300 mb-3 leading-tight">{t('my_plan.ai.subtitle')}</p>
            <div className="flex gap-2">
                <input type="text" value={goal} onChange={(e) => setGoal(e.target.value)} placeholder={t('my_plan.ai.placeholder')} className="flex-grow p-2 bg-gray-900 border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:outline-none text-sm" />
                <button onClick={handleGenerateJourney} disabled={isLoading} className="bg-hey-church-red text-white px-3 rounded-lg font-bold disabled:bg-gray-700 transition-colors">
            ...
            {journey && <div className="mt-3 p-3 bg-gray-900/50 rounded-lg border border-gray-700/50 max-h-32 overflow-y-auto"><p className="whitespace-pre-wrap text-gray-300 text-xs">{journey}</p></div>}
        </ActionCard>
      </div>
