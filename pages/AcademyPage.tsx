...
import { PlayCircle, Lock, CheckCircle, ChevronDown, ArrowLeft, Loader2, BookOpen, Play } from 'lucide-react';
        ...
        <div className="relative w-[85vw] md:w-[360px] h-[75vh] flex-shrink-0 bg-black rounded-3xl overflow-hidden shadow-2xl border border-gray-800 snap-center mx-2 flex flex-col justify-center">
            ...
            <div className="absolute bottom-0 left-0 right-0 p-6 flex flex-col items-start z-10 pointer-events-auto">
                <div className="mb-4 w-full">
                    <span className="bg-hey-church-red text-white text-[10px] font-bold px-2 py-1 rounded-md uppercase tracking-wider mb-2 inline-block shadow-md">
                        Hey Shot
                    </span>
                    <h2 className="text-2xl font-black text-white uppercase leading-tight drop-shadow-lg max-w-[95%]">
                        {session.title}
                    </h2>
                </div>
...
const CourseCard: React.FC<{ course: Course; isLocked: boolean }> = ({ course, isLocked }) => {
                ...
                <div className="flex justify-between items-start mb-6">
                    ...
                    {progress > 0 && (
                        <div className="bg-black/30 backdrop-blur-sm px-3 py-1 rounded-full border border-white/10 flex items-center">
                            <div className="w-2 h-2 rounded-full bg-hey-church-green mr-2 animate-pulse"></div>
        ...
        return (
            <div className="bg-hey-church-bg min-h-screen text-white p-4 pb-24">
                ...
                <div className="space-y-4">
                    {filteredCourses.map(course => <CourseCard key={course.id} course={course} isLocked={!course.isPublished} />)}
                        ...
                        <div className="flex overflow-x-auto space-x-4 pb-4 scrollbar-hide snap-x">
                            {heyShotSessions.map((session) => (
                                <div key={session.id} onClick={() => setActivePathId('hey_shots')} className="flex-shrink-0 w-32 h-56 rounded-xl overflow-hidden relative shadow-lg cursor-pointer group snap-start bg-gradient-to-br from-red-800 to-red-950 border border-white/10 transition-transform hover:scale-105">
                                    ...
                                    <img src={session.thumbnail || `https://img.youtube.com/vi/${session.videoId}/hqdefault.jpg`} alt={session.title} className="absolute inset-0 w-full h-full object-cover opacity-20 mix-blend-overlay grayscale group-hover:opacity-30 transition-opacity" />
                                    <div className="absolute inset-0 p-3 flex flex-col justify-between z-10">
                                        ...
                                        <div><p className="text-white text-sm font-extrabold uppercase leading-tight drop-shadow-md line-clamp-4">{session.title}</p></div>
                                    </div>
                                </div>
                            ))}
                        </div>
