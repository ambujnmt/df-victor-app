...
const LeaderDashboardPage: React.FC = () => {
    ...
    if (loading) return <div className="min-h-screen bg-hey-church-bg flex items-center justify-center text-white"><Loader2 className="animate-spin h-8 w-8"/></div>;

    return (
        <div className="p-4 space-y-6 bg-hey-church-bg min-h-screen text-white pb-24 font-sans">
            <div className="flex justify-between items-center">
                <button onClick={() => navigate('/my-plan')} className="flex items-center text-hey-church-orange-400 font-bold hover:text-white transition-colors">
                    <ArrowLeft size={16} className="mr-2"/> Back to Profile
                </button>
                <div className="flex items-center space-x-2">
                    <ShieldCheck size={14} className="text-hey-church-blue" />
                    <div className="text-[10px] bg-hey-church-blue/10 text-hey-church-blue px-3 py-1 rounded-full font-black uppercase tracking-widest border border-hey-church-blue/20">Leader Mode</div>
                </div>
            </div>
            ...
            <div className="bg-gray-800 p-1 rounded-[2rem] shadow-2xl border border-white/5 relative overflow-hidden">
                <div className="bg-gray-900 rounded-[1.8rem] p-6">
                    <div className="flex justify-between items-center mb-4">
                        <h2 className="text-xs font-black flex items-center uppercase tracking-widest text-gray-500"><Users className="mr-2 text-hey-church-orange-400" size={16}/> Select Group to Manage</h2>
                        <button onClick={() => setIsCreatingFam(!isCreatingFam)} className="text-[9px] bg-hey-church-green text-white px-4 py-2 rounded-xl flex items-center font-black uppercase tracking-[0.1em] shadow-lg active:scale-95 transition-all">
                    ...
                    {ledGroups.length > 0 ? (
                        <div className="relative">
                            <select 
                                ...
                                className="w-full p-4 bg-gray-800 border border-gray-700 rounded-2xl text-white font-black text-lg italic outline-none focus:border-hey-church-blue appearance-none pr-10"
                            >
                ...
                {isCreatingFam && (
                    <div className="p-6 space-y-3 bg-hey-church-bg/30 border-t border-white/5 animate-in fade-in slide-in-from-top-2">
                        <p className="text-[10px] font-black text-hey-church-green uppercase tracking-widest">New Group Setup</p>
                        <input placeholder="Group Name" value={newFam.name} onChange={e => setNewFam({...newFam, name: e.target.value})} className="w-full p-4 rounded-2xl bg-gray-800 text-white border border-gray-700 outline-none focus:border-hey-church-green font-bold"/>
                        ...
                        <button onClick={handleCreateFam} disabled={isSaving} className="w-full bg-hey-church-green text-white font-black py-4 rounded-2xl uppercase tracking-widest shadow-xl hover:scale-[1.02] active:scale-95 transition-all">
                ...
                <div className="space-y-6 animate-in fade-in duration-500">
                    ...
                    <div className="bg-gradient-to-br from-gray-800 to-gray-900 p-1 rounded-[2.5rem] shadow-2xl border border-white/5 relative overflow-hidden">
                        <div className="bg-gray-900/60 p-8 rounded-[2.3rem] flex items-start justify-between relative z-10 backdrop-blur-sm">
                            <div className="flex items-center">
                                <div className="relative">
                                    <img src={selectedGroup.avatar} className="w-24 h-24 rounded-[2rem] border-4 border-hey-church-blue object-cover shadow-2xl mr-6"/>
                                    <div className="absolute -bottom-2 -right-2 bg-hey-church-green text-white p-1.5 rounded-lg border-2 border-gray-900 shadow-xl"><CheckCircle size={14}/></div>
                                </div>
                                <div>
                                    <div className="flex items-center space-x-2">
                                        <h2 className="text-3xl font-black text-white italic tracking-tighter leading-none">{selectedGroup.name}</h2>
                                    </div>
                                    <p className="text-xs text-hey-church-blue font-black uppercase tracking-[0.2em] mt-2 flex items-center">
                                        <Users size={14} className="mr-2"/> {groupMembers.length} ACTIVE MEMBERS
                                    </p>
                                </div>
                            </div>
                            <button onClick={openEditModal} className="bg-gray-800 hover:bg-gray-700 text-hey-church-orange-400 p-4 rounded-3xl transition-all shadow-xl active:scale-90 border border-white/5"><Settings size={24}/></button>
                        </div>
                    </div>
                    ...
                    <div className="bg-gray-800 p-8 rounded-[2.5rem] shadow-2xl space-y-8 border border-white/5">
                         <div className="flex justify-between items-end border-b border-gray-700 pb-4">
                            <h2 className="text-xl font-black uppercase italic tracking-tighter flex items-center">
                                <MessageSquare className="mr-3 text-hey-church-orange-400" size={26}/> Command Center
                            </h2>
                            <span className="text-[8px] font-black text-gray-500 uppercase tracking-widest">Group ID: {selectedGroup.id.slice(-6)}</span>
                         </div>
                         
                         <div className="space-y-8">
                            <div className="bg-hey-church-blue/5 p-6 rounded-[2rem] border border-hey-church-blue/20 flex items-center justify-between group">
                                <div>
                                ...
                                <button 
                                    ...
                                    className="bg-hey-church-blue text-white p-4 rounded-2xl shadow-xl hover:scale-105 active:scale-95 transition-all flex items-center font-black uppercase text-[10px] tracking-widest"
                                >
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex gap-3">
                                    <input value={announcement} onChange={e => setAnnouncement(e.target.value)} placeholder="Message to everyone in group..." className="flex-grow p-4 bg-gray-900 border border-gray-700 rounded-2xl text-white text-sm outline-none focus:border-hey-church-red transition-all font-medium"/>
                                    <button onClick={handleSendAnnouncement} className="bg-hey-church-red p-4 rounded-2xl text-white hover:brightness-110 shadow-lg active:scale-95 transition-all"><Send size={20}/></button>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-4">
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        <button onClick={handleSuggestChallenge} className="bg-hey-church-orange-500 p-4 rounded-2xl text-white hover:brightness-110 shadow-lg active:scale-95 transition-all"><Award size={20}/></button>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        <button onClick={handleAssignCourse} className="bg-hey-church-green p-4 rounded-2xl text-white hover:brightness-110 shadow-lg active:scale-95 transition-all"><BookOpen size={20}/></button>
                                    </div>
                                </div>
                            </div>
                         </div>
                    ...
                    <div className="bg-gray-800 p-8 rounded-[2.5rem] shadow-2xl border border-white/5 space-y-8">
                        <div className="flex justify-between items-center">
                            ...
                            <span className="bg-black/40 px-4 py-1.5 rounded-full text-[10px] font-black text-hey-church-yellow border border-white/5">{groupMembers.length} IN CIRCLE</span>
                        </div>

                        {/* Direct Member Search Tool */}
                        <div className="space-y-4">
                            <label className="text-[10px] font-black text-hey-church-orange-400 uppercase tracking-[0.3em] ml-2 flex items-center">
                            ...
                            <div className="relative">
                                <input 
                                    ...
                                    className="w-full p-4 bg-gray-900 border border-gray-700 rounded-2xl text-white text-sm outline-none focus:border-hey-church-blue"
                                />
                                {userSearchQuery && (
                                        ...
                                        {searchableUsers.length > 0 ? (
                                            searchableUsers.map(u => (
                                                <button 
                                                    ...
                                                    className="w-full p-4 flex items-center justify-between hover:bg-hey-church-blue/10 transition-colors border-b border-gray-700 last:border-0"
                                                >
                                                    <div className="flex items-center">
                                                    ...
                                                    <UserCheck size={18} className="text-hey-church-blue"/>
                                                </button>
                                            ))
                                        ) : (
                                ...
                                <div key={m.id} className="flex items-center justify-between p-4 bg-black/20 rounded-[1.5rem] border border-white/5 hover:bg-black/30 transition-all group">
                                    <div className="flex items-center">
                                        ...
                                        <div className="flex flex-col">
                                            ...
                                            <span className="text-[8px] text-hey-church-orange-400 font-black uppercase tracking-widest mt-1.5">
                    ...
                    <div className="bg-gray-800 w-full max-w-md rounded-[3rem] shadow-[0_40px_100px_rgba(0,0,0,0.8)] p-10 relative border border-white/10 overflow-y-auto max-h-[90vh]">
                        ...
                        <div className="mb-10 text-center">
                             <h2 className="text-3xl font-black uppercase italic flex items-center justify-center mb-2">
                                <Settings className="mr-3 text-hey-church-orange-400" size={32}/> Group Settings
                             </h2>
                        ...
                        <div className="space-y-8">
                            <div className="flex flex-col items-center">
                                <div className="relative mb-4">
                                    <img src={editForm.avatar} className="w-32 h-32 rounded-[2.5rem] object-cover border-4 border-hey-church-blue shadow-2xl"/>
                                    <button 
                                        onClick={() => fileInputRef.current?.click()} 
                                        disabled={isProcessingImage}
                                        className="absolute -bottom-2 -right-2 p-3 bg-hey-church-blue text-white rounded-2xl shadow-2xl hover:scale-105 active:scale-90 transition-all border-4 border-gray-800"
                                    >
                            ...
                            <div className="space-y-6">
                                <div>
                                    ...
                                    <input value={editForm.name || ''} onChange={e => setEditForm({...editForm, name: e.target.value})} className="w-full p-5 bg-gray-900 rounded-[1.5rem] text-white font-black text-xl italic outline-none focus:border-hey-church-blue border border-gray-700 shadow-inner" placeholder="Group Name"/>
                                </div>
                            ...
                            <button 
                                ...
                                disabled={isSaving}
                                className="w-full bg-hey-church-green text-white font-black py-6 rounded-[2rem] mt-4 flex items-center justify-center shadow-[0_20px_50px_rgba(16,185,129,0.3)] hover:scale-[1.02] active:scale-95 transition-all uppercase tracking-[0.2em] text-lg"
                            >
