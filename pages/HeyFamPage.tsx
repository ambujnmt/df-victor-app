...
const HeyFamPage: React.FC = () => {
    ...
    if (loading) return <div className="min-h-screen bg-hey-church-bg flex items-center justify-center"><Loader2 className="animate-spin text-white"/></div>;
    if (!fam) return <div className="p-4 text-white">Group not found.</div>;

    return (
        <div className="bg-hey-church-bg min-h-screen flex flex-col text-white">
            {/* Header */}
            <div className="p-4 border-b border-gray-700 bg-gray-800/80 backdrop-blur-md sticky top-0 z-10">
                <div className="flex items-center justify-between mb-4">
                    <button onClick={() => navigate('/community')} className="text-hey-church-orange-400"><ArrowLeft /></button>
                    <div className="flex space-x-3">
                        {isLeader && (
                             <button onClick={openEditModal} className="bg-gray-700 p-2 rounded-full hover:bg-gray-600 text-hey-church-orange-400">
                                <Settings size={20}/>
                             </button>
                        )}
                        {isMember && !isLeader && (
                ...
                <div className="flex items-center space-x-4">
                    <img src={fam.avatar} alt={fam.name} className="w-16 h-16 rounded-full border-2 border-hey-church-orange-500 object-cover"/>
                    <div className="flex-grow">
                        <h1 className="text-xl font-bold">{fam.name}</h1>
                        <p className="text-sm text-gray-400">{fam.memberIds.length} Members</p>
                    </div>
                     {!isMember && (
                        <button onClick={handleJoin} className="bg-hey-church-red px-4 py-2 rounded-full font-bold">Join</button>
                    )}
                </div>
                
                {isMember && (
                    <div className="flex mt-4 bg-gray-700 rounded-lg p-1">
                        <button onClick={() => setActiveTab('chat')} className={`flex-1 py-2 rounded-md text-sm font-bold flex items-center justify-center ${activeTab === 'chat' ? 'bg-hey-church-blue text-white' : 'text-gray-400'}`}>
                            <MessageSquare size={16} className="mr-1"/> Chat
                        </button>
                        <button onClick={() => setActiveTab('info')} className={`flex-1 py-2 rounded-md text-sm font-bold flex items-center justify-center ${activeTab === 'info' ? 'bg-hey-church-blue text-white' : 'text-gray-400'}`}>
                            <Info size={16} className="mr-1"/> Info
                        </button>
                         <button onClick={() => setActiveTab('members')} className={`flex-1 py-2 rounded-md text-sm font-bold flex items-center justify-center ${activeTab === 'members' ? 'bg-hey-church-blue text-white' : 'text-gray-400'}`}>
                                    ...
                                    if (isAnnouncement) {
                                        return (
                                            <div key={i} className="flex justify-center my-4">
                                                <div className="bg-hey-church-yellow/20 border border-hey-church-yellow text-hey-church-yellow px-4 py-2 rounded-lg text-sm text-center max-w-xs">
                                    ...
                                    return (
                                        <div key={i} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                                            <div className={`flex max-w-[80%] ${isMe ? 'flex-row-reverse' : 'flex-row'}`}>
                                                ...
                                                <div className={`p-3 rounded-2xl ${isMe ? 'bg-hey-church-blue text-white rounded-br-none' : 'bg-gray-700 text-gray-200 rounded-bl-none'}`}>
                                                    {!isMe && <p className="text-xs font-bold text-hey-church-orange-400 mb-1">{msg.userName}</p>}
                        ...
                        <div className="p-4 space-y-6">
                            <div className="bg-gray-800 p-4 rounded-xl shadow-lg">
                                ...
                                <div className="mt-4 space-y-2">
                                    <div className="flex items-center text-gray-300">
                                        <Clock size={18} className="mr-3 text-hey-church-orange-400"/>
                                        <span>{fam.meetingTime || 'Time TBD'}</span>
                                    </div>
                                    <div className="flex items-center text-gray-300">
                                        <MapPin size={18} className="mr-3 text-hey-church-orange-400"/>
                            ...
                            <div className="bg-gray-800 p-4 rounded-xl shadow-lg">
                                <div className="flex justify-between items-center mb-3">
                                    ...
                                    {isLeader && <button onClick={() => setIsAddingResource(!isAddingResource)} className="text-hey-church-green"><Plus size={20}/></button>}
                                </div>
                                ...
                                {isAddingResource && (
                                    <div className="bg-gray-700 p-3 rounded-lg mb-4 space-y-2">
                                        ...
                                        <button onClick={handleAddResource} className="w-full bg-hey-church-green py-2 rounded font-bold text-sm">Add</button>
                                    </div>
                                )}
                                    ...
                                    {fam.resources?.length > 0 ? fam.resources.map(res => {
                                        ...
                                        return (
                                            <a key={res.id} href={res.url} target="_blank" rel="noopener noreferrer" className="flex items-center p-3 bg-gray-700 rounded-lg hover:bg-gray-600 transition">
                                                <Icon size={20} className="mr-3 text-hey-church-orange-400"/>
                                ...
                                {participants.map((p, i) => {
                                    ...
                                    return (
                                        <div key={p.id} className={`flex items-center p-4 ${i !== participants.length -1 ? 'border-b border-gray-700' : ''}`}>
                                            ...
                                            <div className="flex-grow">
                                                <p className="font-bold">{p.name} {p.id === fam.leaderId && <span className="text-xs bg-hey-church-yellow text-black px-2 py-0.5 rounded-full ml-2">Leader</span>}</p>
                                            </div>
                                        </div>
                                    );
                                })}
                 ...
                 <div className="fixed bottom-[72px] left-0 right-0 p-3 bg-gray-900 border-t border-gray-700">
                    <div className="max-w-lg mx-auto flex items-center gap-2">
                         <input 
                            ...
                            className="flex-grow p-3 bg-gray-800 border border-gray-600 rounded-full text-white placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-hey-church-orange-400"
                            onKeyPress={(e) => e.key === 'Enter' && handleSendMessage()}
                        />
                        <button onClick={handleSendMessage} disabled={isSending || !newMessage.trim()} className="bg-hey-church-red p-3 rounded-full text-white disabled:bg-gray-700">
                            <Send size={20}/>
                        </button>
                    </div>
                </div>
            ...
            {isEditing && (
                <div className="fixed inset-0 bg-black/70 flex items-center justify-center p-4 z-50">
                    <div className="bg-gray-800 rounded-xl shadow-2xl max-w-lg w-full p-6 relative">
                        ...
                        <h2 className="text-xl font-bold mb-4 flex items-center"><Settings className="mr-2 text-hey-church-orange-400"/> Edit Group Settings</h2>
                        <div className="space-y-4">
                            ...
                            <button onClick={handleSaveEdit} className="w-full bg-hey-church-red font-bold py-3 rounded-lg mt-2 flex items-center justify-center">
