...
const DataSyncPage: React.FC = () => {
    ...
    return (
        <div className="min-h-screen bg-hey-church-bg flex flex-col items-center justify-center p-4 text-white">
            <div className="bg-gray-800 p-8 rounded-xl shadow-lg text-center my-4 border border-hey-church-orange-900 max-w-2xl w-full">
                <RefreshCw size={40} className="mx-auto text-hey-church-orange-400 mb-4" />
                ...
                <button
                    onClick={handleSync}
                    disabled={isSyncing}
                    className="mt-6 w-full bg-hey-church-red text-white font-bold py-4 px-4 rounded-lg text-lg transition-transform hover:scale-105 disabled:bg-gray-600 disabled:scale-100 flex items-center justify-center"
                >
                    {isSyncing ? (
                        <>
